...
class RealmResource<Payload: RealmEndpointObject>: Endpoint<Payload> {

    let config: Realm.Configuration
    let dataTags: [DataTaggable]
    let type: RealmEndpointObject.Type
    private let extractClosure: ((Realm) -> Results<Payload>)?
    private let postParse: ((Payload, Realm) -> Void)?
    
    override var paging: Bool {         // single realm objects don't page
        return false
    }
...
extension RealmResource: RealmResultsProvider {
